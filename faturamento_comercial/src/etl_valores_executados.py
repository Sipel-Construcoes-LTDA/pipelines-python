...
def find_header_row(file_path: str, encoding: str = 'utf-16') -> int:
    ...
    try:
        ...
        logger.warning(
            f"Erro ao ler linhas para encontrar header em {file_path}: {e}")
    return 0
...
def load_dim_values(zrm_map: pd.DataFrame, year_label: str) -> pd.DataFrame:
    ...
    merged['Chave composta (FK)'] = merged['Chave composta (FK)'].fillna(
        '').astype(str)
...
def process_pipeline() -> None:
    ...
    all_files: List[str] = glob.glob(os.path.join(
        VALORES_DIR, "**", "*.XLS"), recursive=True)
    ...
    for file_path in all_files:
        try:
            ...
            df_temp = pd.read_csv(file_path, sep='\t',
                                  encoding='utf-16', skiprows=header_row)
    ...
    full_df['CenTrabRes'] = full_df['CenTrabRes'].fillna(
        '').astype(str).str.strip()
    full_df = pd.merge(full_df, dim_ct, left_on='CenTrabRes',
                       right_on='NOMECLATURA_BD_IW69', how='left')

    full_df['BASE OPERACIONAL'] = full_df['BASE OPERACIONAL'].fillna(
        '').astype(str).str.strip()
    full_df['PK'] = full_df['BASE OPERACIONAL'] + full_df['ChaveFK']

    full_df = pd.merge(full_df, dim_val_2024, left_on='PK',
                       right_on='FK_FINAL', how='left')
    full_df = pd.merge(full_df, dim_val_2025, left_on='PK',
                       right_on='FK_FINAL', how='left')

    group_cols: List[str] = ["Nota", "Data", "BASE OPERACIONAL"]
    additional_cols: List[str] = ["Fim avaria",
                                  "Texto code para codificação", "Local"]
    ...
    final_df = final_df[~((final_df['Valor Total 2024'] == 0)
                          & (final_df['Valor Total 2025'] == 0))]
    logger.info(
        f"Linhas removidas (valores zerados): {initial_len - len(final_df)}")

    # Output
    output_path = os.path.join(
        PROCESSED_DIR, "faturamentos_executados_consolidado.csv")
