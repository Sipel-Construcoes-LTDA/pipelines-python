...
def extract_and_clean_jacobina(file_path: str) -> pd.DataFrame:
    ...
    df.rename(columns={"Gestor no Tangerino": "Gestor", "Cód. ": "Cód."}, inplace=True)
    df["Base"] = "Jacobina"
...
def extract_and_clean_juazeiro(file_path: str) -> pd.DataFrame:
    ...
    df.rename(
        columns={"GESTOR": "Gestor", "STATUS ": "STATUS", "Código Sistema": "Cód."},
        inplace=True,
    )
    df["Base"] = "Juazeiro"
...
def unify_colaboradores() -> pd.DataFrame:
    ...
    df_colaboradores = pd.concat(
        [df_bonfim, df_jacobina, df_juazeiro], ignore_index=True, sort=False
    )
...
def load_and_prep_gestores() -> pd.DataFrame:
    ...
    try:
        ...
        df["Clean_Name"] = (
            df["INSPETORES"].astype(str).str.split(" -").str[0].str.strip().str.title()
        )
        # Garante que a coluna alvo está formatada
        df["PRIMEIRO E ULTIMO NOME"] = (
            df["PRIMEIRO E ULTIMO NOME"].astype(str).str.title().str.strip()
        )
...
def standardize_managers(
    df_processed: pd.DataFrame, df_gestores: pd.DataFrame
) -> pd.DataFrame:
    ...
    target_name_map: Dict[str, str] = dict(
        zip(df_gestores["Clean_Name"], df_gestores["PRIMEIRO E ULTIMO NOME"])
    )
    ...
    def get_standard_manager(name: Any) -> str:
        ...
        # Limiar de 80% para variações como "Alan Moura" -> "Alan Santos De Moura"
        if best_match and best_match[1] >= 80:
            standard_name = target_name_map[best_match[0]]
            manager_cache[name_clean] = standard_name
            logging.info(
                f"[Gestor Match] '{name}' -> '{standard_name}' (Score: {best_match[1]})"
            )
            return standard_name
    ...
    df_processed["Gestor_Padronizado"] = df_processed["Gestor"].apply(
        get_standard_manager
    )
    return df_processed
...
def process_descontos(df_colaboradores: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    logging.info(
        f"Processando arquivo de descontos com Fuzzy Matching em cascata: {DESCONTOS_FILE}"
    )
    ...
    df_descontos["Nome_Original"] = (
        df_descontos["Nome_Original"].str.strip().str.title()
    )
    ...
    for nome_desconto in nomes_descontos:
        ...
        if best_match and best_match[1] >= SIMILARITY_THRESHOLD:
            ...
            if nome_desconto != best_match[0]:
                logging.info(
                    f"[ETAPA 1] Correspondência: '{nome_desconto}' -> '{best_match[0]}' (Score: {best_match[1]})"
                )
        else:
    ...
    gestor_map: Dict[str, List[str]] = (
        df_colaboradores.groupby("Gestor")["Nome"].apply(list).to_dict()
    )

    df_unmatched = df_descontos[
        df_descontos["Nome_Original"].isin(unmatched_after_primary)
    ]
    ...
    for _, row in df_unmatched.iterrows():
        ...
        if pd.notna(supervisor) and supervisor in gestor_map:
            ...
            if best_match and best_match[1] >= SIMILARITY_THRESHOLD:
                nome_map[nome_desconto] = best_match[0]
                logging.info(
                    f"[ETAPA 2-S] '{nome_desconto}' -> '{best_match[0]}' via Supervisor '{supervisor}' (Score: {best_match[1]})"
                )
                found_match = True
        ...
        if not found_match and pd.notna(coordenador) and coordenador in gestor_map:
            ...
            if best_match and best_match[1] >= SIMILARITY_THRESHOLD:
                nome_map[nome_desconto] = best_match[0]
                logging.info(
                    f"[ETAPA 2-C] '{nome_desconto}' -> '{best_match[0]}' via Coordenador '{coordenador}' (Score: {best_match[1]})"
                )
                found_match = True
    ...
    if final_unmatched:
        logging.warning(
            "Após todas as etapas, os seguintes colaboradores NÃO foram encontrados:"
        )
        for name in sorted(list(set(final_unmatched))):
            logging.warning(f" - {name}")
    else:
        logging.info(
            "Todos os colaboradores foram correspondidos com sucesso após todas as etapas!"
        )
    ...
    df_processed["Data Ocorrência"] = pd.to_datetime(
        df_processed["Data Ocorrência"], format="%d/%m/%Y", errors="coerce"
    )
    df_processed = df_processed[df_processed["Data Ocorrência"] > "2024-12-31"]

    setor_replacements: Dict[str, str] = {
        "Linha Viva Juazeiro 1": "Linha Viva",
        "Manutenção Pesada": "Manutenção",
        "Comercial Juazeiro": "Comercial",
        "Comercial C. Alegre": "Comercial",
        "Turma Rubem": "Construção",
        "Comercial Remanso": "Comercial",
        "Linha": "Linha Viva",
        "Linha Viva Viva": "Linha Viva",
    }
    ...
    # Gestor removido daqui pois já foi tratado
    cols_to_capitalize: List[str] = ["Origem", "Tipo", "Grupo Item", "Item", "Setor"]
    for col in cols_to_capitalize:
        if col in df_processed.columns:
            # Preenche NA com string vazia antes de converter e capitalizar para evitar "Nan"
            df_processed[col] = (
                df_processed[col].fillna("").astype(str).str.title().str.strip()
            )

    df_processed["Grupo Item"] = df_processed["Grupo Item"].replace(
        ["", "Nan"], "Não Especificado"
    )
