...
def load_and_prep_gestores() -> pd.DataFrame:
    ...
    try:
        ...
        df['Clean_Name'] = df['INSPETORES'].astype(
            str).str.split(' -').str[0].str.strip().str.title()
        # Garante que a coluna alvo está formatada
        df['PRIMEIRO E ULTIMO NOME'] = df['PRIMEIRO E ULTIMO NOME'].astype(
...
def process_descontos(df_colaboradores: pd.DataFrame) -> pd.DataFrame:
    ...
    df_descontos['Nome_Original'] = df_descontos['Nome_Original'].str.strip(
    ).str.title()
    ...
    df_descontos['Coordenador'] = df_descontos['Coordenador'].str.strip(
    ).str.title()

    # --- 1. Fuzzy Matching Primário (Nome do Colaborador) ---
    logging.info(
        "Iniciando 1ª Etapa: Correspondência aproximada por nome (Limiar 96).")
    ...
    for nome_desconto in nomes_descontos:
        best_match = fuzzy_process.extractOne(
            nome_desconto, nomes_colaboradores)
    ...
    gestor_map: Dict[str, List[str]] = df_colaboradores.groupby(
        'Gestor')['Nome'].apply(list).to_dict()

    df_unmatched = df_descontos[df_descontos['Nome_Original'].isin(
    ...
    cols_to_capitalize: List[str] = [
        "Origem", "Tipo", "Grupo Item", "Item", "Setor"]
    ...
    df_processed['Item'] = df_processed['Item'].replace(
        ["", "Nan"], "Não Especificado")
...
def main() -> None:
    ...
    try:
        ...
        logging.info(
            f"Pipeline concluído com sucesso. Arquivo salvo em: {OUTPUT_FILE}")
