...
def transform_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for src, target in date_cols_map.items():
        df[target] = df[src].apply(convert_custom_date) if src in df.columns else pd.NaT
    df["Criação da nota"] = df["InícioAvar"].apply(convert_custom_date) if "InícioAvar" in df.columns else pd.NaT
    ...
    cols_to_remove = ["Concl.desj", "ContContr.", "Instalação", "Dt.criação", "H fim des.", "TensFornec", "Code", "Data", "Hora", "Poste", "Grp.cod", "UnLeit.", "Ordenação", "TensMed", "CNAE", "EqMedVizAn", "EqMedVizPo", "MedVizPos", "MedVizAnt", "Posto", "InícioAvar", "Fim avaria", "HFimAvar", "Encerram.", "HEnc.", "Modif.em", "Ordem", "C", "HInícAv.", "Centro cst", "Den.exec.", "Den.exec._1", "Exec.por", "Execução", "P", "Rg", "Localiz.", "Cen.", "Bairro de", "   DiasExec", "QtdHorExec", "Centro de Resultado", "Municípo", "Nome do parceiro", " DiasExec", "  DiasExec", "TpPri", "Cliente", "Texto breve", "Ctg.tar.", "NºEndereço", "Column14", "DiasExec"]
    ...
    if "Urbano/Rur" in df.columns:
        df["Urbano/Rur"] = df["Urbano/Rur"].fillna("Info.").replace({"R": "Rural", "U": "Urbano"})
