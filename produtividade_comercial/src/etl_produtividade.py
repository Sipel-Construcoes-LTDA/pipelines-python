...
def read_file(filepath: str) -> pd.DataFrame:
    ...
    try:
        df = pd.read_csv(
            filepath, sep="\t", encoding="utf-16", skiprows=3, on_bad_lines="skip"
        )
...
def transform_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Criação da nota"] = (
        df["InícioAvar"].apply(convert_custom_date)
    ...
    df["Nº do pedido"] = (
        df["Nº do pedido"].fillna("Vazio") if "Nº do pedido" in df.columns else "Vazio"
    )
    ...
    if "Urbano/Rur" in df.columns:
        df["Urbano/Rur"] = (
            df["Urbano/Rur"].fillna("Info.").replace({"R": "Rural", "U": "Urbano"})
        )
