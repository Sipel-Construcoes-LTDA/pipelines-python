    ...
    for col in cols_date:
        if col in df.columns:
            df[col] = pd.to_datetime(
                df[col], errors='coerce', dayfirst=False, utc=True).dt.tz_localize(None).dt.normalize()

    for col in cols_str:
        if col in df.columns:
            df[col] = df[col].astype(str).str.strip()
            replace_mask = df[col].str.lower().isin(
                ['nan', 'nat', 'none', '<na>', ''])
...
def main() -> None:
    try:
        ...
        df_mat = pd.read_csv(path_materiais, sep=';',
                             dtype=str, encoding='utf-8-sig')
        df_res = pd.read_csv(path_reservas, sep=';',
                             dtype=str, encoding='utf-8-sig')

        mapping_mat = {'Quant_x002e_Confirmada': 'Quant_Confirmada',
                       'DataRegularisado': 'DataRegularizacao'}
        df_mat = df_mat.rename(columns=mapping_mat)
        df_res = df_res.rename(
            columns={'DataRegularisado': 'DataRegularizacao'})
        ...
        if 'Id' in df_final.columns:
            ...
            df_final['Id'] = df_final['Id'].astype(str).str.strip().replace(
                ['nan', 'NaN', 'None', 'nan', ''], np.nan)
        ...
        if len(group_cols) >= 3 and 'IdSolic' in df_final.columns:
            ...
            if mask_enc.any():
                df_final.loc[mask_enc, 'obra'] = df_final.loc[mask_enc, 'obra'].astype(
                    str).str.strip().replace(r'^[Bb]-', '', regex=True).str.strip()
                reserva_serie = df_final.loc[mask_enc, 'NumeroReserva'].astype(
                    str).replace(['nan', 'NaN', 'None', '<NA>', ''], 'S-RES')
                df_final.loc[mask_enc, 'IdSolic'] = ("ENC-" + df_final.loc[mask_enc, 'Created_Date'].dt.strftime(
                    '%Y%m%d') + "-" + df_final.loc[mask_enc, 'obra'].astype(str) + "-" + df_final.loc[mask_enc, 'TipoSolic'].astype(str) + "-" + reserva_serie)

        if 'StatusSolic' in df_final.columns:
            status_map = {'Confirmado': 'Movimentado', 'Reservado': 'Pendente', 'Deletado': 'Rejeitado',
                          'Estornado': 'Rejeitado', 'Mov Parcial': 'Mov. Parcial', 'Mov.Parcial': 'Mov. Parcial'}
        ...
        if 'Justificar' in df_final.columns:
            df_final['Justificar'] = df_final['Justificar'].fillna(
                'Comum').replace({'': 'Comum', 'Normal': 'Comum'})

        if 'titulo' in df_final.columns:
            df_final['titulo'] = df_final['titulo'].astype(
                str).str.title().str.strip()

        cols_to_drop = ['MotivoRejSolic', 'EmailSolic', 'IsAvulso', 'LatitudeSolic', 'LongetudeSolic', 'Observacao', 'isUrgente', 'UnidadeMedida',
                        'ProjetoFilho', 'Separacao', 'AuthorId', 'EditorId', 'MOTIVO_EXCLUSAO', 'AVALIACAO_MATERIAL', 'CENTRO_MATERIAL', 'RECEBEDOR', 'SUPR_MATR', 'GUID']
        df_final.drop(
            columns=[c for c in cols_to_drop if c in df_final.columns], inplace=True)

        subset_validation = ['IdSolic', 'obra', 'CodMaterial']
        subset_validation = [
            c for c in subset_validation if c in df_final.columns]

        if subset_validation:
            initial_rows = len(df_final)
            mask = df_final[subset_validation].replace(
                '', np.nan).isna().all(axis=1)
        ...
        df_final.to_csv(output_path, sep=';', index=False,
                        encoding='utf-8-sig', date_format='%Y-%m-%d', decimal=',')
