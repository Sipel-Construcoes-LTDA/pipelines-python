...
def main() -> None:
    try:
        ...
        mapping_mat = {'Quant_x002e_Confirmada': 'Quant_Confirmada', 'DataRegularisado': 'DataRegularizacao'}
        ...
        if len(group_cols) >= 3 and 'IdSolic' in df_final.columns:
            ...
            if mask_enc.any():
                df_final.loc[mask_enc, 'obra'] = df_final.loc[mask_enc, 'obra'].astype(str).str.strip().replace(r'^[Bb]-', '', regex=True).str.strip()
                reserva_serie = df_final.loc[mask_enc, 'NumeroReserva'].astype(str).replace(['nan', 'NaN', 'None', '<NA>', ''], 'S-RES')
                df_final.loc[mask_enc, 'IdSolic'] = ("ENC-" + df_final.loc[mask_enc, 'Created_Date'].dt.strftime('%Y%m%d') + "-" + df_final.loc[mask_enc, 'obra'].astype(str) + "-" + df_final.loc[mask_enc, 'TipoSolic'].astype(str) + "-" + reserva_serie)

        if 'StatusSolic' in df_final.columns:
            status_map = {'Confirmado': 'Movimentado', 'Reservado': 'Pendente', 'Deletado': 'Rejeitado', 'Estornado': 'Rejeitado', 'Mov Parcial': 'Mov. Parcial', 'Mov.Parcial': 'Mov. Parcial'}
        ...
        cols_to_drop = ['MotivoRejSolic', 'EmailSolic', 'IsAvulso', 'LatitudeSolic', 'LongetudeSolic', 'Observacao', 'isUrgente', 'UnidadeMedida', 'ProjetoFilho', 'Separacao', 'AuthorId', 'EditorId', 'MOTIVO_EXCLUSAO', 'AVALIACAO_MATERIAL', 'CENTRO_MATERIAL', 'RECEBEDOR', 'SUPR_MATR', 'GUID']
