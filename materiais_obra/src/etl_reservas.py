...
def extract_list_items_paged(ctx: ClientContext, list_name: str, page_size: int = 5000) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        items: ListItemCollection = target_list.items.paged(
            page_size).get().execute_query()
...
def clean_pendency_tags(text: Any) -> str:
    ...
    parts = [p.strip().title()
             for p in str(text).replace(',', ';').split(';') if p.strip()]
...
def clean_and_profile_data(raw_data: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    cols_to_drop = [c for c in df.columns if c.startswith(
        'odata.') or c.startswith('__')]
    system_cols = ['FileSystemObjectType', 'ServerRedirectedEmbedUri', 'ServerRedirectedEmbedUrl', 'ContentTypeId',
                   'ComplianceAssetId', 'Attachments', 'AssRespAlmoxarifado', 'AssRespSaque', 'GUID', 'OData__ColorTag', 'OData__UIVersionString']
    ...
    if id_col:
        df[id_col] = df[id_col].astype(str).replace(
            ['nan', 'NaN', 'None', ''], pd.NA)
        df.dropna(subset=[id_col], inplace=True)
    else:
        logger.error("Coluna de Identificação (Id/ID) não encontrada!")

    date_candidates = ['Created', 'Modified'] + [col for col in df.columns if (
        'Data' in col or 'DATE' in col.upper()) and col not in system_cols]
    ...
    if 'DataRegularisado' in df.columns:
        df.rename(
            columns={'DataRegularisado': 'DataRegularizacao'}, inplace=True)
...
def apply_business_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'DataCriacaoReserva' in df.columns:
        if 'DataSolicPrev' not in df.columns:
            ...
            df['DataSolicPrev'] = df['DataSolicPrev'].fillna(
                df['DataCriacaoReserva'])

    df['Processo'] = 'Encerramento'

    if 'obra' in df.columns:
        df['obra'] = df['obra'].astype(str).str.strip().str.replace(
            r'^[Bb]-', '', regex=True).str.strip()
    ...
    if len(group_cols) >= 3 and 'IdSolic' in df.columns:
        ...
        reserva_serie = df['NumeroReserva'].astype(str).replace(
            ['nan', 'NaN', 'None', '<NA>', ''], 'S-RES')
        df['IdSolic'] = ("ENC-" + df['Created_Date'].dt.strftime('%Y%m%d') + "-" +
                         df['obra'].astype(str) + "-" + df['TipoSolic'].astype(str) + "-" + reserva_serie)
    ...
    if 'Quant_Confirmada' in df.columns:
        df['Quant_Confirmada'] = pd.to_numeric(
            df['Quant_Confirmada'], errors='coerce').fillna(0)

    if 'NumeroReserva' in df.columns:
        df['NumeroReserva'] = pd.to_numeric(
            df['NumeroReserva'], errors='coerce')

    if 'Pendencias' in df.columns:
        df['Pendencias'] = df['Pendencias'].apply(clean_pendency_tags)

    cols_to_normalize = ['titulo', 'Descricao', 'Observacao', 'StatusSolic', 'Coleborador_solicitante',
                         'AgenteResponsavel', 'obra', 'BaseOperacional', 'DescricaoPendencias', 'isUrgente', 'IsDeleted']
    for col in [c for c in cols_to_normalize if c in df.columns]:
        df[col] = df[col].astype(str).str.title().str.strip().replace(
            {'Nan': '', 'Nat': '', 'None': '', '<Na>': ''}, regex=False)

    if 'obra' in df.columns:
        df['obra'] = pd.to_numeric(
            df['obra'], errors='coerce').round().astype('Int64')
        df.dropna(subset=['obra'], inplace=True)

    if 'StatusSolic' in df.columns:
        status_map = {'Confirmado': 'Movimentado',
                      'Mov Parcial': 'Mov. Parcial', 'Mov.Parcial': 'Mov. Parcial'}
        df['StatusSolic'] = df['StatusSolic'].replace(status_map)

    df.dropna(axis=1, how='all', inplace=True)

    int_cols = ['NumeroReserva', 'CodMaterial', 'TipoSolic',
                'CENTRO_MATERIAL', 'idCoordenador', 'idSupervisor']
...
def standardize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mapping = {
        'PROJETO': 'obra', 'CODIGO': 'CodMaterial', 'DESCRICAO_MATERIAL': 'MaterialSolic',
        'BASE': 'BaseOperacional', 'TIPO_MOVIMENTACAO': 'TipoSolic', 'STATUS': 'StatusSolic',
        'DATA_CRIACAOCOELBA': 'DataCriacaoReserva', 'NUMERO_RESERVA': 'NumeroReserva',
        'DATA_SOLICITACAO': 'DataSolic', 'QUANTIDADE_SOLICITADA': 'QuantSolic',
        'QUANTIDADE_MOVIMENTADA': 'Quant_Confirmada', 'IDSolic': 'IdSolic', 'Titulo': 'titulo',
        'DataMovimentacao': 'DataSolicMod', 'TECNICO': 'Coleborador_solicitante', 'Urgente': 'isUrgente',
        'OBSERVACAO_MAT': 'Observacao', 'Descri_x00e7__x00e3_oPendencias': 'DescricaoPendencias'
    }
...
def resolve_status_aggr(series: pd.Series) -> str:
    ...
    statuses = {s.strip().title() for s in series.astype(
        str) if s.strip() and s.lower() not in ['nan', 'none', 'nat']}
    if not statuses:
        return 'Pendente'
...
def create_reservas_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    agg_rules = {'StatusSolic': resolve_status_aggr, 'QuantSolic': 'sum', 'Quant_Confirmada': 'sum',
                 'Pendencias': agg_pendencias_aggr, 'DescricaoPendencias': agg_descriptions_aggr}

    metadata_cols = ['obra', 'BaseOperacional', 'TipoSolic',
                     'titulo', 'DataSolic', 'DataCriacaoReserva', 'Processo']
    for col in [c for c in metadata_cols if c in df.columns]:
        agg_rules[col] = 'first'

    date_cols = [c for c in df.columns if 'Data' in c or c in [
        'Created', 'Modified']]
...
def consolidate_all_data(df_reservas: pd.DataFrame, root_dir: Path) -> pd.DataFrame:
    ...
    if not materiais_path.exists():
        logger.warning(
            f"Arquivo de materiais não encontrado: {materiais_path}")
        return pd.DataFrame()

    df_mat = pd.read_csv(materiais_path, sep=';',
                         encoding='utf-8-sig', low_memory=False)
    mat_mapping = {'Quant_x002e_Confirmada': 'Quant_Confirmada',
                   'Coleborador_solicitante': 'Solicitante', 'IdSolic': 'IdSolic'}
    df_mat = df_mat.rename(columns=mat_mapping)
    df_all = pd.concat([df_mat, df_reservas], ignore_index=True)

    date_cols = [c for c in df_all.columns if 'Data' in c or c in [
        'Created', 'Modified']]
    for col in date_cols:
        df_all[col] = pd.to_datetime(
            df_all[col], errors='coerce', utc=True).dt.tz_localize(None).dt.normalize()

    if 'Created' in df_all.columns:
        df_all['Created_Date_Temp'] = pd.to_datetime(
            df_all['Created']).dt.normalize()
        mask_enc = df_all['Processo'] == 'Encerramento'
        if mask_enc.any():
            logger.info(
                "Padronizando IdSolic na base consolidada (Encerramento) com prefixo ENC-...")
            df_all.loc[mask_enc, 'obra'] = df_all.loc[mask_enc, 'obra'].astype(
                str).str.strip().str.replace(r'^[Bb]-', '', regex=True).str.strip()
            reserva_serie = df_all.loc[mask_enc, 'NumeroReserva'].astype(
                str).replace(['nan', 'NaN', 'None', '<NA>', ''], 'S-RES')
            df_all.loc[mask_enc, 'IdSolic'] = ("ENC-" + df_all.loc[mask_enc, 'Created_Date_Temp'].dt.strftime(
                '%Y%m%d') + "-" + df_all.loc[mask_enc, 'obra'].astype(str) + "-" + df_all.loc[mask_enc, 'TipoSolic'].astype(str) + "-" + reserva_serie)
    ...
    if 'QuantSolic' in df_all.columns:
        df_all['QuantSolic'] = pd.to_numeric(
            df_all['QuantSolic'], errors='coerce')
        df_all.dropna(subset=['QuantSolic'], inplace=True)

    overlap_mapping = {'Quant_x002e_Confirmada': 'Quant_Confirmada', 'QUANTIDADE_MOVIMENTADA': 'Quant_Confirmada',
                       'QUANTIDADE_SOLICITADA': 'QuantSolic', 'PROJETO': 'obra', 'TIPO_MOVIMENTACAO': 'TipoSolic', 'STATUS': 'StatusSolic', 'Status': 'StatusSolic'}
    ...
    int_cols_final = ['obra', 'CodMaterial', 'TipoSolic', 'CENTRO_MATERIAL',
                      'NumeroReserva', 'idCoordenador', 'idSupervisor', 'AuthorId', 'EditorId']
    for col in [c for c in int_cols_final if c in df_all.columns]:
        df_all[col] = pd.to_numeric(
            df_all[col], errors='coerce').astype('Int64')

    logger.info("Agrupando base consolidada geral...")

    agg_rules = {'QuantSolic': 'sum', 'Quant_Confirmada': 'sum', 'StatusSolic': resolve_status_aggr,
                 'Pendencias': agg_pendencias_aggr, 'DescricaoPendencias': agg_descriptions_aggr}
    for col in df_all.columns:
        if col in agg_rules or col == 'IdSolic':
            continue
        agg_rules[col] = 'max' if 'Data' in col or col in [
            'Created', 'Modified'] or pd.api.types.is_numeric_dtype(df_all[col]) else 'first'
    ...
    cols_to_remove = ['LatitudeSolic', 'LongetudeSolic', 'Id', '', '_1', '_2', '_3', 'GUID', 'MOTIVO_EXCLUSAO', 'AVALIACAO_MATERIAL', 'CENTRO_MATERIAL',
                      'RECEBEDOR', 'SUPR_MATR', 'AuthorId', 'EditorId', 'IsAvulso', 'isUrgente', 'Observacao', 'MotivoRejSolic', 'ID', 'DATA_CRIACAO', 'DataEstorno']
    df_final.drop(columns=[
                  c for c in cols_to_remove if c in df_final.columns], inplace=True, errors='ignore')

    if 'IsDeleted' in df_final.columns:
        df_final['IsDeleted'] = df_final['IsDeleted'].astype(str).str.lower().map(
            {'0': False, '0.0': False, 'false': False, '1': True, '1.0': True, 'true': True}).fillna(False).astype(bool)
    ...
    if 'StatusSolic' in df_final.columns:
        df_final['StatusSolic'] = df_final['StatusSolic'].replace(
            {'Confirmado': 'Movimentado', 'Reservado': 'Pendente', 'Deletado': 'Rejeitado', 'Estornado': 'Rejeitado'})

    if 'titulo' in df_final.columns:
        df_final['titulo'] = df_final['titulo'].astype(
            str).str.title().str.strip().replace('Nan', '', regex=False)

    if 'QuantSolic' in df_final.columns:
        df_final['QuantSolic'] = pd.to_numeric(
            df_final['QuantSolic'], errors='coerce')
    ...
    text_cols = [c for c in df_final.select_dtypes(
        include=['object']).columns if not ('Data' in c or c in ['Created', 'Modified'])]
    for col in text_cols:
        df_final[col] = df_final[col].astype(str).str.replace(';', ',', regex=False).str.replace(
            '\n', ' ', regex=False).str.replace('\r', '', regex=False).str.strip()
        df_final.loc[df_final[col].str.lower().isin(
            ['nan', 'none', 'nat', '']), col] = ''
...
def main() -> None:
    ...
    try:
        ...
        if not df_treated.empty:
            df_treated.to_csv(output_path_raw, index=False, sep=';',
            ...
            df_grouped_res.to_csv(output_dir_proc / "tabela_reservas_agrupada.csv", index=False,
                                  sep=';', encoding='utf-8-sig', date_format='%Y-%m-%d', decimal=',')
            logger.info("Tabela de reservas agrupada salva.")

            df_consolidated = consolidate_all_data(df_treated, root_dir)
            if not df_consolidated.empty:
                df_consolidated.to_csv(output_dir_proc / "solicitacoes_consolidadas_geral.csv",
                                       index=False, sep=';', encoding='utf-8-sig', date_format='%Y-%m-%d', decimal=',')
