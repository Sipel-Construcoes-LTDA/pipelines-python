...
def clean_and_profile_data(raw_data: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    system_cols = ['FileSystemObjectType', 'ServerRedirectedEmbedUri', 'ServerRedirectedEmbedUrl', 'ContentTypeId', 'ComplianceAssetId', 'Attachments', 'AssRespAlmoxarifado', 'AssRespSaque', 'GUID', 'OData__ColorTag', 'OData__UIVersionString']
    ...
    date_candidates = ['Created', 'Modified'] + [col for col in df.columns if ('Data' in col or 'DATE' in col.upper()) and col not in system_cols]
...
def apply_business_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(group_cols) >= 3 and 'IdSolic' in df.columns:
        ...
        df['IdSolic'] = ("ENC-" + df['Created_Date'].dt.strftime('%Y%m%d') + "-" + df['obra'].astype(str) + "-" + df['TipoSolic'].astype(str) + "-" + reserva_serie)
    ...
    cols_to_normalize = ['titulo', 'Descricao', 'Observacao', 'StatusSolic', 'Coleborador_solicitante', 'AgenteResponsavel', 'obra', 'BaseOperacional', 'DescricaoPendencias', 'isUrgente', 'IsDeleted']
    ...
    if 'StatusSolic' in df.columns:
        status_map = {'Confirmado': 'Movimentado', 'Mov Parcial': 'Mov. Parcial', 'Mov.Parcial': 'Mov. Parcial'}
...
def consolidate_all_data(df_reservas: pd.DataFrame, root_dir: Path) -> pd.DataFrame:
    ...
    mat_mapping = {'Quant_x002e_Confirmada': 'Quant_Confirmada', 'Coleborador_solicitante': 'Solicitante', 'IdSolic': 'IdSolic'}
    ...
    if 'Created' in df_all.columns:
        ...
        if mask_enc.any():
            ...
            df_all.loc[mask_enc, 'IdSolic'] = ("ENC-" + df_all.loc[mask_enc, 'Created_Date_Temp'].dt.strftime('%Y%m%d') + "-" + df_all.loc[mask_enc, 'obra'].astype(str) + "-" + df_all.loc[mask_enc, 'TipoSolic'].astype(str) + "-" + reserva_serie)
    ...
    overlap_mapping = {'Quant_x002e_Confirmada': 'Quant_Confirmada', 'QUANTIDADE_MOVIMENTADA': 'Quant_Confirmada', 'QUANTIDADE_SOLICITADA': 'QuantSolic', 'PROJETO': 'obra', 'TIPO_MOVIMENTACAO': 'TipoSolic', 'STATUS': 'StatusSolic', 'Status': 'StatusSolic'}
    ...
    int_cols_final = ['obra', 'CodMaterial', 'TipoSolic', 'CENTRO_MATERIAL', 'NumeroReserva', 'idCoordenador', 'idSupervisor', 'AuthorId', 'EditorId']
    ...
    cols_to_remove = ['LatitudeSolic', 'LongetudeSolic', 'Id', '', '_1', '_2', '_3', 'GUID', 'MOTIVO_EXCLUSAO', 'AVALIACAO_MATERIAL', 'CENTRO_MATERIAL', 'RECEBEDOR', 'SUPR_MATR', 'AuthorId', 'EditorId', 'IsAvulso', 'isUrgente', 'Observacao', 'MotivoRejSolic', 'ID', 'DATA_CRIACAO', 'DataEstorno']
    ...
    if 'titulo' in df_final.columns:
        df_final['titulo'] = df_final['titulo'].astype(str).str.title().str.strip().replace('Nan', '', regex=False)
    ...
    text_cols = [c for c in df_final.select_dtypes(include=['object']).columns if not ('Data' in c or c in ['Created', 'Modified'])]
