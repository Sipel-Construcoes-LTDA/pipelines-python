...
def get_sharepoint_context(url: str) -> ClientContext:
    ...
    try:
        if client_id and client_secret:
            ...
            raise ValueError(
                "Nenhuma credencial válida encontrada no .env para App-Only ou User/Password.")
...
def clean_and_profile_data(raw_data: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    system_cols = ['FileSystemObjectType', 'ServerRedirectedEmbedUri', 'ServerRedirectedEmbedUrl',
                   'ContentTypeId', 'ComplianceAssetId', 'Attachments', 'AssRespAlmoxarifado', 'AssRespSaque']
    ...
    date_candidates = ['Created', 'Modified'] + \
        [col for col in df.columns if 'Data' in col or 'DATE' in col.upper()]
...
def apply_business_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ['QuantSolic', 'Quant_x002e_Confirmada']:
        if col in df.columns:
            ...
            if outliers_mask.any():
                logger.warning(
                    f"Coluna '{col}' possui {outliers_mask.sum()} valores > 10000. Zerando-os.")
                df.loc[outliers_mask, col] = 0

    if 'Pendencias' in df.columns:
        df['Pendencias'] = df['Pendencias'].apply(clean_pendency_tags)

    cols_to_normalize = ['Coleborador_solicitante', 'IsDeleted', 'BaseOperacional', 'Observacao',
                         'isUrgente', 'AgenteResponsavel', 'DescricaoPendencias', 'Justificativa', 'Justificar']
    ...
    int_cols = ['CodMaterial', 'TipoSolic', 'CENTRO_MATERIAL',
...
def create_solicitations_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.empty or 'IdSolic' not in df.columns:
        logger.error(
            "Nenhum dado válido ou coluna 'IdSolic' não encontrada para agrupamento.")
        return pd.DataFrame()
    ...
    def agg_text_unique(series: pd.Series) -> str:
        items = {str(s).strip() for s in series if pd.notna(s) and str(
            s).strip() and str(s).lower() not in ['nan', 'none', '<na>']}
        return " | ".join(sorted(items))

    metadata_cols = ['Coleborador_solicitante', 'obra', 'BaseOperacional', 'isUrgente',
                     'AgenteResponsavel', 'EmailSolic', 'TipoSolic', 'titulo', 'Titulo', 'Processo']
    date_cols = [col for col in df.columns if 'Data' in col or col in [
        'Created', 'Modified']]
    ...
    logger.info(
        f"Agrupamento concluído: {len(df)} linhas -> {len(df_grouped)} solicitações únicas.")
    return df_grouped
...
def main() -> None:
    ...
    try:
        ...
        df_final.to_csv(output_path_raw, index=False, sep=';',
        ...
        if not df_grouped.empty:
            ...
            df_grouped.to_csv(output_path_processed, index=False,
                              sep=';', encoding='utf-8-sig', date_format='%Y-%m-%d')
