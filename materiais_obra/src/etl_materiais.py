...
def apply_business_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "obra" in df.columns:
        df["obra"] = (
            df["obra"].astype(str).str.strip().str.replace(r"^[Bb]-", "", regex=True)
        )
...
def create_solicitations_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def resolve_status(series: pd.Series) -> str:
        statuses = set(
            s.strip() for s in series.astype(str) if pd.notna(s) and str(s).strip()
        )
    ...
    def agg_text_unique(series: pd.Series) -> str:
        items = {
            ...
            if pd.notna(s)
            and str(s).strip()
            and str(s).lower() not in ["nan", "none", "<na>"]
        }
    ...
    date_cols = [
        col for col in df.columns if "Data" in col or col in ["Created", "Modified"]
    ]

    agg_rules: Dict[str, Any] = {  # <-- Added type hint here
